    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['help:helpReport:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['help:helpReport:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['help:helpReport:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['help:helpReport:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="helpReportList" @selection-change="handleSelectionChange">
...
<!--        </template>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="帮扶数量" prop="helpCount">
          <el-input v-model="form.helpCount" placeholder="请输入帮扶数量" />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        helpCount: [
          { required: true, message: "帮扶数量不能为空", trigger: "blur" }
        ],
        averageScore: [
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getHelpReport(id).then(response => {
        this.loading = false;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateHelpReport(this.form).then(response => {
            ...
            addHelpReport(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除帮扶报表中心编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delHelpReport(ids);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('help/helpReport/export', {
        ...this.queryParams
      }, `helpReport_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
